...
var usernsTests = []struct {
	...
	{
		`^RKT_BIN^ --debug --insecure-skip-verify run ^USERNS^ --set-env=FILE=^FILE^ --mds-register=false ^IMAGE^`,
		"/",
		"drwxrwxr-x",
		"0",
		"0",
	...
	{
		`^RKT_BIN^ --debug --insecure-skip-verify run --set-env=FILE=^FILE^ --mds-register=false ^IMAGE^`,
		"/proc/1/root/",
		"drwxrwxr-x",
		"0",
...
func TestUserns(t *testing.T) {
	...
	for i, tt := range usernsTests {
		for _, userNsOpt := range []string{"", "--no-overlay --private-users"} {
			runCmd := tt.runCmd
